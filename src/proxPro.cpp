...
namespace ppro {
	...
	std::string globals() {
		...
		for (ind i = 0; i < s; ++i) {
			...
			else if (GRegister_Variables[i]->type == 1) {//IVAR Type 1: Basic Interface Type
			...
			else if (GRegister_Variables[i]->type == 2) {//IVAR Type 2: Bounded Type
				variables::ivariable* v = static_cast<variables::ivariable*>(GRegister_Variables[i].get());
				ss << v->name << " = DefineNumber[";
				ss << v->value << ", ";
				if (v->menu.exists) {
			...
			else if (GRegister_Variables[i]->type == 3) {//IVAR Type 3:Choice Type
				variables::ivariable* v = static_cast<variables::ivariable*>(GRegister_Variables[i].get());
				ss << v->name << " = DefineNumber[";
				ss << v->value << ", ";
				if (v->menu.exists) {
			...
			else if (GRegister_Variables[i]->type == -1) {//VAR Type -1:Pairs List Type
				variables::variable* v = GRegister_Variables[i].get();
				ss << v->name << " = {";
				for (ind j = 0; j < v->spairs.size(); ++j) {
					ss << v->spairs[j].one << ", " << v->spairs[j].two;
					ss << ",\n\t\t";
				}
				ss.seekp(-4, ss.cur);
				ss << "};\n";
